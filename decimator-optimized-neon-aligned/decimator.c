...
static void fastmixer(float *sout_re, float *sout_im, const float *sinp_re, const float *sinp_im, struct _mixer *mixer, int len) {
    ...
    for (int i=0; i<len; i+=4) {
        ...
        s_in_re[0] = sinp_re[i  ];
        s_in_re[1] = sinp_re[i+1];
        s_in_re[2] = sinp_re[i+2];
        s_in_re[3] = sinp_re[i+3];

        s_in_im[0] = sinp_im[i  ];
        s_in_im[1] = sinp_im[i+1];
        s_in_im[2] = sinp_im[i+2];
        s_in_im[3] = sinp_im[i+3];

        //VECTYPE *s_out_re = (VECTYPE *)&sout_re[i];
        //VECTYPE *s_out_im = (VECTYPE *)&sout_im[i];

        VECTYPE s_out_re = s_in_re * osc_re - s_in_im * osc_im;
        VECTYPE s_out_im = s_in_re * osc_im + s_in_im * osc_re;

        sout_re[i  ] = s_out_re[0];
        sout_re[i+1] = s_out_re[1];
        sout_re[i+2] = s_out_re[2];
        sout_re[i+3] = s_out_re[3];

        sout_im[i  ] = s_out_im[0];
        sout_im[i+1] = s_out_im[1];
        sout_im[i+2] = s_out_im[2];
        sout_im[i+3] = s_out_im[3];
...
int fastdecimator(struct _sample *sampleout, const struct _sample *samplein, struct _decimstate *ds) {
    ...
    for (j=0; j <= ds->samplelen - 4*ds->coefflen4 - ds->decimfactor; j+= ds->decimfactor) {
        ...
        for (int i=0; i < ds->coefflen4; i++) {
            ...
            s_re[0] = ds->sample_re[4*i + j    ];
            s_re[1] = ds->sample_re[4*i + j + 1];
            s_re[2] = ds->sample_re[4*i + j + 2];
            s_re[3] = ds->sample_re[4*i + j + 3];

            s_im[0] = ds->sample_im[4*i + j    ];
            s_im[1] = ds->sample_im[4*i + j + 1];
            s_im[2] = ds->sample_im[4*i + j + 2];
            s_im[3] = ds->sample_im[4*i + j + 3];
